...
class DashboardStatsController extends Controller
{
     ...
     *
     * Uses RedisWorkloadRepository::processing() when available. Otherwise derives
     * the state from WorkloadRepository::get() reserved counts so contract-only
     * implementations are not forced to add methods and do not report a misleading idle state.
     *
     * @return bool
     */
    ...
    /**
     * Get the exact active-batch count for sidebar navigation.
     *
     * Active means not cancelled and still carrying unsettled work
     * (pending_jobs > failed_jobs). Returns null when database batching is
     * unavailable (e.g. DynamoDB or a missing table). Previews remain
     * dashboard-only via /api/batches/overview.
     *
     * @return int|null
     */
     ...
     *
     * Uses RedisJobRepository::countFailedSince() when available. Otherwise returns
     * null so the UI can render an unavailable state without requiring the method
     * on the public JobRepository contract.
     *
     ...
     * Uses RedisJobRepository::countFailedSince() when available. Otherwise returns
     * null so the UI can render an unavailable state without requiring the method
     * on the public JobRepository contract. Also returns null when failed-job trim
     * is shorter than a full day so an incomplete window is never presented as 24h.
     *
     * @param  \Laravel\Horizon\Contracts\JobRepository  $jobs
     * @return int|null
     */
