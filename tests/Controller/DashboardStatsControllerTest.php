...
class DashboardStatsControllerTest extends ControllerTest
{
    public function test_all_stats_are_correctly_returned()
    {
        ...
        // Concrete repository mock keeps countFailedSince available without putting
        // that helper on the public JobRepository contract.
        ...
        // Top-level batches overview must stay out of the global stats payload.
    ...
    public function test_navigation_batch_count_returns_exact_active_batches()
    {
        ...
        // Active predicate: cancelled_at null AND pending_jobs > failed_jobs.
        // Four+ qualifying actives so a count capped to three previews cannot pass.
    ...
    public function test_failed_jobs_past_hour_is_null_when_failed_trim_is_below_one_hour()
    {
        ...
        // Concrete mock keeps countFailedSince present so the trim threshold is the gate under test.
        $jobs = Mockery::mock(RedisJobRepository::class);
        $jobs->shouldReceive('countRecentlyFailed')->andReturn(1);
    ...
    {
        // Trim keeps more than one hour of failed jobs but less than a full day.
    ...
    /**
     * Stub navigation repositories used by every /api/stats request.
     *
     * @param  bool  $withEmptyBatchTable
     * @return void
     */
    ...
    /**
     * Configure an in-memory job_batches table for scalar navigation counts.
     *
     * @return void
     */
